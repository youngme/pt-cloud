package com.bin.cloud.gateway.web.filter;
 ...
 */
@Configuration
@ComponentScan(basePackages = "com.bin.cloud.auth.client")
@Slf4j
public class AccessGatewayFilter implements GlobalFilter {

    private final static String X_CLIENT_TOKEN_USER = "x-client-token-core";
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        if (authService.invalidJwtAccessToken(authentication)) {
            return unauthorizedToken(exchange);
        }
        // 如果请求未携带token信息, 直接跳出
     ...
     * @param
     */
    private Mono<Void> unauthorizedToken(ServerWebExchange serverWebExchange) {

            serverWebExchange.getResponse().setStatusCode(HttpStatus.OK);
            DataBuffer buffer = serverWebExchange.getResponse()
                    .bufferFactory().wrap(Result.fail(SystemErrorType.FEIGN_TIME_OUT).toUnTokenString().getBytes());
            return serverWebExchange.getResponse().writeWith(Flux.just(buffer));
    }
}
